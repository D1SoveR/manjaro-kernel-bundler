...
def generate_bundle_for_preset(preset, sb_dir, generate_fallback=True):
	...
	with TempFileMap(
		...
		if (
			os.path.isfile(os.path.join(sb_dir, "db.key")) and
			os.path.isfile(os.path.join(sb_dir, "db.crt")) and
		):

			run([
				"sbsign",
				"--key",  os.path.join(sb_dir, "db.key"),
				"--cert", os.path.join(sb_dir, "db.crt"),
				"--output", output_file,
				tmpfiles.bundle.name
			])
			...
			copyfile(mpfiles.bundle.name, output_file)
